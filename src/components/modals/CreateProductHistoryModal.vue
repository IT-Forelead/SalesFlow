    ...
    <template v-slot:body>
      <div class="w-full relative mb-4">
        ...
        <input type="text" v-model="search" :placeholder="t('searchByProductNameOrBarcode')"
          class="bg-slate-100 border-none dark:text-white rounded-lg w-full h-12 pl-10 placeholder-slate-400"
          @input="isOpen = true">
        <ul v-if="searchResults.length && isOpen"
          class="mt-1 w-full max-h-60 overflow-y-auto absolute z-10 dark:bg-slate-600 border border-slate-200 rounded-xl shadow-md">
      ...
      <div v-if="selectedProduct" class="space-y-4">
        <div class="w-full mb-10">
          ...
          <div class="py-4 border-b border-slate-200 flex items-center justify-between">
            ...
            <p class="text-sm leading-none dark:text-white">
          ...
          <div class="py-4 border-b border-slate-200 flex items-center justify-between">
            ...
            <p class="text-sm leading-none dark:text-white mr-3">
        ...
        <div class="flex items-center space-x-4">
          <div class="flex-1 space-y-1">
            <label for="default-value"
              class="block mb-2 dark:text-white text-base font-normal after:text-red-500 after:content-['*']">
              {{ $t('quantity') }}
            </label>
            <input id="default-value" type="text" v-model="submitData.quantity"
              class="bg-slate-50 border border-slate-200 dark:text-white text-base rounded-2xl focus:ring-green-400/40 focus:border-green-400/40 focus:ring-4 block w-full p-2.5"
        ...
        <div class="flex items-center justify-between space-x-4">
          <div class="flex-1 space-y-1">
            ...
            <money3 id="price" v-bind="moneyConf" v-model.number="submitData.purchasePrice"
              class="bg-slate-50 text-right border border-slate-200 dark:text-white text-base rounded-2xl focus:ring-green-400/40 focus:border-green-400/40 focus:ring-4 block w-full p-2.5">
            </money3>
          </div>
          <div class="flex-1 space-y-1">
            <label for="price"
