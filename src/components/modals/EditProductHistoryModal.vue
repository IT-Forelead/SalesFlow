  ...
  <CModal :is-open="useModalStore().isOpenEditProductHistoryModal" v-if="useModalStore().isOpenEditProductHistoryModal"
    ...
    <template v-slot:body>
      <div class="space-y-4">
        ...
        <div class="flex items-center space-x-4">
          <div class="flex-1 space-y-1">
            <label for="price" class="block text-left mb-2 text-slate-900 dark:text-white text-base font-medium after:text-red-500 ">
              {{ $t('purchasePrice') }}
            </label>
            <money3 id="price" v-bind="moneyConf" v-model.number="submitData.purchasePrice"
              class="bg-slate-50 text-gray-900 dark:bg-slate-700 dark:text-white text-right border border-slate-200 text-base rounded-2xl focus:ring-green-400/40 focus:border-green-400/40 focus:ring-4 block w-full p-2.5">
            </money3>
          </div>
          <div class="flex-1 space-y-1">
            <label for="price" class="block text-left mb-2 text-slate-900 dark:text-white text-base font-medium after:text-red-500 ">
            ...
            <money3 id="price" v-bind="moneyConf" v-model.number="submitData.salePrice"
              class="bg-slate-50 dark:bg-slate-700 dark:text-white text-right border border-slate-200 text-slate-900 text-base rounded-2xl focus:ring-green-400/40 focus:border-green-400/40 focus:ring-4 block w-full p-2.5">
            </money3>
          </div>
        </div>
        <div class="flex items-center space-x-4">
          ...
          <div class="flex-1 space-y-1">
            ...
            <select id="default-type" v-model="submitData.productSaleType"
              class="bg-slate-100 dark:bg-slate-700 dark:text-white border-none text-slate-900 rounded-lg text-base md:text-lg block w-full h-11">
        ...
        <div class="space-x-2 space-y-1 md:text-left text-center">
          <button v-for="months in [1, 3, 6, 9, 12, 24]" :key="months" @click="calculateExpirationDate(months)" type="button"
            class="text-slate-600 dark:bg-slate-800 bg-white dark:text-white hover:bg-slate-100 hover:dark:bg-slate-900 focus:ring-4 focus:outline-none focus:ring-slate-300 rounded-xl border border-slate-200 text-sm font-medium px-2.5 py-1 hover:text-slate-900 hover:dark:text-white lowercase">+
        ...
        <div class="flex space-x-10">
          ...
          <div v-else class="text-base dark:text-white md:text-lg font-medium">
            <label>{{ $t('percent') + ": " }}</label>
            <span class="font-light">{{ calcPercentOfSale(submitData.purchasePrice, submitData.salePrice) + "%" }}</span>
          </div>
          <div class="text-base dark:text-white md:text-lg font-medium">
            <label>{{ $t('sold') + ": " }}</label>
