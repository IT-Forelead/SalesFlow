...
const closeModal = () => {
  useModalStore().closeInvestInfoModal()
...
const getBalanceHistoriesByFilter = (filter) => {
  ...
  CustomerService.getCustomerHistoriesByFilter(
    cleanObjectEmptyFields({
      customerId: useInvestStore().selectedClient.customerId,
      type: filter,
    }),
  ).then((res) => {
...
<template>
  <CModal :is-open="useModalStore().isOpenInvestInfoModal" v-if="useModalStore().isOpenInvestInfoModal" @close=closeModal>
