...
<template>
  ...
  >
    <template #container>
      <div class="flex flex-col px-8 py-8 gap-6 bg-white dark:bg-slate-800 animate__animated animate__bounceIn"
        ...
        <div class="p-4 flex flex-col justify-center items-center" id="pricing">
          <div class="ring-2 ring-green-500 rounded-3xl w-full p-8 xl:p-10">
            ...
            <p class="mt-6 flex items-center flex-col space-y-4">
              <span class="line-through text-4xl font-sans text-gray-500/70 dark:text-zinc-300/70">{{ useMoneyFormatter(totalPrice) }}</span>
              <span class="text-5xl font-bold tracking-tight text-gray-900 dark:text-zinc-200">{{ useMoneyFormatter(calculateDiscountPrice) }}</span>
            </p>
          </div>
        </div>
      </div>
