  ...
  <main class="flex items-center h-screen overflow-hidden bg-gray-50 dark:bg-[#0D1117]">
    ...
    <div class="mx-auto flex w-full max-w-2xl flex-col px-4 sm:px-6" v-if="isOrderExists ">
      <div class="relative mt-12 sm:mt-16 z-[1]">
        ...
        <div class="mt-3 text-center text-lg dark:text-white dark:text-[#e6edf3]">
        </div>
      </div>
      <div class="-mx-2 z-[2] mt-10 flex-auto dark:bg-slate-600 dark:bg-[#161B22] px-4 py-10 shadow-2xl shadow-gray-900/10 dark:shadow-gray-500/10 sm:mx-0 sm:flex-none sm:rounded-5xl sm:p-24 rounded-3xl">
        <div class="space-y-6">
          <div>
            <label for="fullName" class="mb-2 block text-base font-semibold text-gray-900 dark:text-[#e6edf3]"> {{ $t('fullName')}} </label>
            <input v-model="submitData.fullName" type="text" id="fullName" class="border appearance-none text-sm rounded-lg block w-full p-2.5 dark:bg-slate-600 dark:bg-[#0D1117] border-gray-200 dark:border-[#30363D] placeholder-gray-400 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500" :placeholder="t('enterFullName')" />
          </div>
          <div>
            <label for="phone" class="mb-2 block text-base font-semibold text-gray-900 dark:text-[#e6edf3]"> {{ $t('phone')}}</label>
            <div class="relative">
              <input v-model="submitData.phone" id="phone" v-maska data-maska="+998(##) ###-##-##" class="border appearance-none text-sm rounded-lg block w-full p-2.5 dark:bg-slate-600 dark:bg-[#0D1117] border-gray-200 dark:border-[#30363D] placeholder-gray-400 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="+998(00) 000-00-00" />
            </div>
          </div>
        </div>
